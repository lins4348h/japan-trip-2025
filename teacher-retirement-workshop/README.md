...
| 檔案 | 用途 |
| --- | --- |
| `../open-slide-workspace/slides/teacher-retirement-cashflow/index.tsx` | open-slide 簡報原始檔（52 頁，含講者備註與每頁時間；實作頁用學習單範例版截圖） |
...
| `web/` | 簡報網頁版（open-slide 匯出後執行 `python3 patch_web_deck.py`，補上逐步呈現、換頁淡入，以及滑動、滾輪、點擊與按鈕翻頁） |
| `學習單_我的退休帳本.pdf` | 學習單列印版（7 頁 A4，含 A～G 總複習表） |
| `簡報_教師退休現金流工作坊.pptx` | 可編輯 PowerPoint 版（尚為 55 頁舊版，需要時再重新產生；文字框、色塊可直接改，含講者備註；學習單截圖與 QR Code 為圖片）。由 `pptx_export/`（extract.cjs 量網頁版每頁版面 → build.cjs 用 pptxgenjs 組成 PPTX）產生 |
